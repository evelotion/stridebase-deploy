...
async function main() {
  ...
  const stores = await prisma.store.createManyAndReturn({
    data: [
      { id: "store-01-pro", name: "CleanStep Express (PRO)", location: "Mall Grand Indonesia, Jakarta", ownerId: "user-mitra-01", storeStatus: "active", tier: "PRO", billingType: "INVOICE", commissionRate: 0, isFeatured: true, rating: 4.8, images: ["process.env.API_BASE_URL + "/uploads/image-1752427316280.jpg"], latitude: -6.1954, longitude: 106.8221 },
      { id: "store-02-basic", name: "Ani Shoe Laundry (BASIC)", location: "Jl. Dago No. 123, Bandung", ownerId: "user-mitra-02", storeStatus: "active", tier: "BASIC", billingType: "COMMISSION", commissionRate: 12.5, isFeatured: false, rating: 4.5, images: ["process.env.API_BASE_URL + "/uploads/image-1752427316280.jpg"], latitude: -6.8943, longitude: 107.6111 },
      { id: "store-03-pending", name: "Tono's Premium Care", location: "Jl. Malioboro, Yogyakarta", ownerId: "user-mitra-03", storeStatus: "pending", tier: "BASIC", billingType: "COMMISSION", commissionRate: 10, isFeatured: false, rating: 0, images: ["process.env.API_BASE_URL + "/uploads/image-1752427316280.jpg"], latitude: -7.7928, longitude: 110.3659 },
      { id: "store-04-inactive", name: "Dina Kicks (Nonaktif)", location: "Jl. Basuki Rahmat, Surabaya", ownerId: "user-mitra-04", storeStatus: "inactive", tier: "BASIC", billingType: "COMMISSION", commissionRate: 10, isFeatured: false, rating: 4.2, images: ["process.env.API_BASE_URL + "/uploads/image-1752427316280.jpg"], latitude: -7.2657, longitude: 112.7399 },
    ]
  });
  ...
  // --- PERBAIKAN DI SINI: Beri ID eksplisit pada review ---
  await prisma.review.create({
    data: {
      id: "review-to-delete-id", // <-- ID INI DITAMBAHKAN
  ...
  // --- PERBAIKAN DI SINI: Payload menggunakan ID review yang benar ---
  await prisma.approvalRequest.create({
    data: {
      requestedById: "user-admin-01",
      actionType: "DELETE_REVIEW",
      // Payload sekarang merujuk ke ID review yang benar-benar ada
  ...
  await prisma.banner.create({ data: { imageUrl: "process.env.API_BASE_URL + "/uploads/asset-1752737852107.png", linkUrl: "/store" } });
