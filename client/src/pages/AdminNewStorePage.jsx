// File: client/src/pages/AdminNewStorePage.jsx (BARU)
...
const AdminNewStorePage = ({ showMessage }) => {
  ...
  const handleOwnerChange = (selectedOption) => {
    setStoreData((prev) => ({ ...prev, ownerId: selectedOption ? selectedOption.value : null }));
  };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showMessage("Toko baru berhasil dibuat dan permintaan persetujuan telah dikirim ke Developer.");
      ...
      <div className="table-card p-4 shadow-sm">
        <form onSubmit={handleSubmit}>
          <div className="mb-3">
            ...
            <input type="text" className="form-control" id="name" name="name" value={storeData.name} onChange={handleChange} required />
          </div>
          <div className="mb-3">
            <label htmlFor="location" className="form-label">Lokasi/Alamat</label>
            <input type="text" className="form-control" id="location" name="location" value={storeData.location} onChange={handleChange} required />
          </div>
          <div className="mb-3">
            <label htmlFor="description" className="form-label">Deskripsi</label>
            <textarea className="form-control" id="description" name="description" rows="4" value={storeData.description} onChange={handleChange}></textarea>
          </div>
          <div className="mb-4">
            <label htmlFor="ownerId" className="form-label">Pemilik Toko (Mitra)</label>
            <Select
              id="ownerId"
              ...
              value={mitraOptions.find(opt => opt.value === storeData.ownerId)}
            />
          </div>
          <div className="text-end">
            <Link to="/admin/stores" className="btn btn-secondary me-2">Batal</Link>
