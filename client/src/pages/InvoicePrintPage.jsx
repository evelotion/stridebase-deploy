// File: stridebase-app/client/src/pages/InvoicePrintPage.jsx

import React, { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import '../invoice.css'; // Kita akan buat file CSS ini
...
const InvoicePrintPage = () => {
    ...
    useEffect(() => {
        if (invoice) {
            // Memicu dialog cetak setelah data dimuat
            window.print();
        }
    }, [invoice]);
            ...
            <div className="invoice-meta">
                <div className="meta-section">
                    ...
                    <address>
                        ...
                        {invoice.store.location}<br />
                        {invoice.store.ownerUser?.email || 'Email tidak tersedia'}
                    </address>
                </div>
                <div className="meta-section text-end">
                    <strong>Tanggal Terbit:</strong> {new Date(invoice.issueDate).toLocaleDateString('id-ID')}<br />
                    <strong>Tanggal Jatuh Tempo:</strong> {new Date(invoice.dueDate).toLocaleDateString('id-ID')}<br />
                    <strong>Status:</strong> <span className="status-paid">{invoice.status}</span>
                </div>
            </div>
