// File: client/src/pages/PaymentSuccessPage.jsx (Dengan Perbaikan Unduh PDF)
...
const PaymentSuccessPage = ({ showMessage }) => {
  ...
  const generatePDF = () => {
    if (!booking) {
      showMessage(
        "Data tanda terima belum siap, silakan coba sesaat lagi.",
        "Error"
      );
      return;
    }
    ...
    doc.text(
      `Tanggal: ${new Date(booking.createdAt).toLocaleString("id-ID")}`,
      14,
      39
    );
  ...
  if (loading)
    return (
      <div className="container py-5 text-center">Memuat tanda terima...</div>
    );
  if (!booking)
    return (
    ...
    <main className="container success-container d-flex align-items-center justify-content-center">
      <div className="success-box text-center">
        ...
        <p className="success-details mb-4">
          Pesanan Anda telah kami konfirmasi. Silakan lacak status pengerjaan di
          dashboard Anda.
        </p>
