// File: client/src/pages/PaymentSimulationPage.jsx (Perbaikan Final)
...
const PaymentSimulationPage = ({ showMessage }) => {
  ...
  const handleCancelBooking = useCallback(async () => {
    ...
    try {
      ...
      navigate("/dashboard"); // Perbaikan kecil: arahkan ke /dashboard
    } catch (error) {
  ...
  useEffect(() => {
    const fetchBooking = async () => {
      ...
      try {
        ...
        // Mengubah 'PENDING' (huruf besar) menjadi 'pending' (huruf kecil)
  ...
  if (!booking) {
    return (
      <div className="container py-5 text-center">
        Gagal memuat detail pesanan untuk pembayaran.
      </div>
    );
  }
    ...
    <main className="container success-container d-flex align-items-center justify-content-center">
      <div className="success-box text-center">
        ...
        <div className="alert alert-info mt-4">
          ...
          Ini adalah halaman simulasi. Jika waktu habis, pesanan akan otomatis
          dibatalkan.
        </div>
      </div>
    </main>
