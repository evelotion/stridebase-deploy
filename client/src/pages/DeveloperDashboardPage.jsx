...
import {
  ...
  reseedDatabase,
  uploadImage, // Pastikan fungsi ini diimpor untuk upload gambar
} from "../services/apiService";

// Daftar font populer untuk dropdown
const googleFonts = [
  "Poppins",
  "Roboto",
  "Open Sans",
...
// Komponen Preview untuk menampilkan perubahan tema secara langsung
const ThemePreview = ({ config }) => {
  // Menerapkan style dinamis dari state config
  const previewStyle = {
    fontFamily: config.typography?.fontFamily || 'sans-serif',
    '--preview-primary-color': config.colors?.primary || '#0d6efd',
    '--preview-secondary-color': config.colors?.secondary || '#6c757d',
    ...
    <div style={previewStyle}>
      ...
      <div className="card">
        <div className="card-body">
          <h5 className="card-title" style={{ color: 'var(--preview-primary-color)' }}>Contoh Judul</h5>
          <p className="card-text" style={{ fontSize: 'var(--preview-font-size-base)' }}>
            Ini adalah contoh teks paragraf yang akan berubah sesuai dengan pengaturan font yang Anda pilih.
          </p>
          <button
...
const DeveloperDashboardPage = ({ showMessage }) => {
  ...
  const handleImageUpload = async (e, path) => {
    ...
    formData.append("image", file); // Backend mengharapkan field 'image'

    try {
      const result = await uploadImage(formData); // Gunakan fungsi upload generik
      const keys = path.split(".");
      setConfig((prevConfig) => {
        const newConfig = JSON.parse(JSON.stringify(prevConfig));
        let current = newConfig;
      ...
      {activeTab === 'theme' && (
        <div className="row g-4">
                    ...
                    <div className="row">
                        <div className="col-md-6">
                            <h5 className="mb-4 fw-bold">Branding & Tampilan</h5>
                             {['logoUrl', 'faviconUrl', 'loginImageUrl', 'registerImageUrl'].map(key => (
                                <div className="mb-3" key={key}>
                                    <label htmlFor={key} className="form-label text-capitalize">{key.replace('Url', ' URL').replace('Image', ' Image ')}</label>
                                    <div className="input-group">
                                        <input type="file" className="form-control" id={key} onChange={(e) => handleImageUpload(e, `branding.${key}`)} accept="image/*" />
                                        {uploadingStatus[`branding.${key}`] && <span className="input-group-text"><div className="spinner-border spinner-border-sm"></div></span>}
                                    </div>
                                    {config.branding[key] && <img src={config.branding[key]} alt={`${key} preview`} className="img-thumbnail mt-2" style={{ maxHeight: '50px' }} />}
                                </div>
                            ))}
                        </div>
                        <div className="col-md-6">
                            ...
                            <div className="mb-3">
                                <label htmlFor="fontFamily" className="form-label">Jenis Font</label>
                                <select className="form-select" id="fontFamily" value={config.typography.fontFamily.split(',')[0].replace(/'/g, '')} onChange={(e) => handleConfigChange({ ...e, target: { ...e.target, value: `'${e.target.value}', sans-serif` } }, "typography.fontFamily")}>
                                    {googleFonts.map(font => <option key={font} value={font}>{font}</option>)}
                                </select>
                            </div>
                            <div className="mb-3">
                                <label htmlFor="baseFontSize" className="form-label">Ukuran Font Dasar: <strong>{config.typography.baseFontSize}</strong></label>
                                <input type="range" className="form-range" id="baseFontSize" min="12" max="20" value={parseInt(config.typography.baseFontSize)} onChange={(e) => handleSliderChange(e, "typography.baseFontSize")} />
                            </div>
                            <div className="mb-3">
                                <label htmlFor="buttonFontSize" className="form-label">Ukuran Font Tombol: <strong>{config.typography.buttonFontSize}</strong></label>
                                <input type="range" className="form-range" id="buttonFontSize" min="12" max="24" value={parseInt(config.typography.buttonFontSize)} onChange={(e) => handleSliderChange(e, "typography.buttonFontSize")} />
                            </div>
                        </div>
                    </div>
                     <hr className="my-4"/>
                     ...
                     <div className="row">
                         {Object.entries(config.featureFlags).filter(([key]) => key !== 'pageStatus').map(([key, value]) => (
                             <div className="col-md-6 mb-3 form-check form-switch" key={key}>
                                 <input className="form-check-input" type="checkbox" role="switch" id={key} checked={!!value} onChange={(e) => handleConfigChange(e, `featureFlags.${key}`)} />
                                 <label className="form-check-label text-capitalize" htmlFor={key}>{key.replace(/([A-Z])/g, ' $1')}</label>
                             </div>
                         ))}
                     </div>
                 </div>
            ...
            <div className="col-lg-4">
                 <div className="card card-account p-3 position-sticky" style={{top: '20px'}}>
      ...
      {activeTab === 'maintenance' && (
        <div className="card card-account p-4">
          ...
          <div className="alert alert-danger">
            <strong>Peringatan:</strong> Aksi di bawah ini akan menghapus semua data transaksi dan mengembalikannya ke kondisi awal. Lanjutkan dengan hati-hati.
          </div>
